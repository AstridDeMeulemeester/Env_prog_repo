...
# Include additional distributions here; however, some distributions will take a while to complete
DISTRIBUTIONS = [norm]  
...
calculate_spi_dataset(dataset, DISTRIBUTIONS, TIMESTEPS)

print("Calculating rejection frequencies")
for distr in DISTRIBUTIONS:
    for timestep in TIMESTEPS:
        rf = calculate_rejfreq(dataset, distr.name, timestep)
        print(f" -> distr={distr.name} -- acc period={timestep} months: {rf*100}%")

print("Calculating and exporting SPEI")
calculate_spei_dataset(dataset, DISTRIBUTIONS, TIMESTEPS, save_path="./output")

# TODO: Plot the lowest and highest SPEI values (from the 10 GCM's) spatially for each time scale

# TODO: Plot the SPEI values spatially for each time scale for yearly average values and seasonal average values

plot_dought_evolution(dataset, norm, 1, "./output")
